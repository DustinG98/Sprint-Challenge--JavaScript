...
const universities = graduates.map(cV => cV.university);
universities.sort()
...
const contactInfo = [];
graduates.forEach(cV => contactInfo.push(`${cV.first_name}, ${cV.email}`))
console.log(contactInfo);

/* Request 3: Find out how many universities have the string "Uni" included in their name. Create a new array called unisWithUni that contains them all. This will be an array of objects. Log the result. */
const unisWithUni = graduates.filter(cV => cV.university.includes("Uni"));
...
*/
const displayNames = [];
zooAnimals.forEach(cV => displayNames.push(`Name: ${cV.animal_name}, Scientific: ${cV.scientific_name}`))
...
*/
const lowPopulationAnimals = zooAnimals.filter(cV => cV.population < 5);
console.log(lowPopulationAnimals);

/* Request 4: .reduce() 

The zoos need to know their total animal population across the United States. 
Find the total population from all the zoos using the .reduce() method. Remember the reduce 
method takes two arguments: a callback (which itself takes two args), and an initial value for the count.

*/
const populationTotal = zooAnimals.reduce((accum, cV) => accum + cV.population, 0)
